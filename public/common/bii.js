...
window.onload = (event) =>{
	...
	setTimeout(function() {
		if(biicore.effect.type == 'heart') {
			return false;
			snowFall.snow(document.getElementsByTagName('body')[0], {image : SNOW_Picture, minSize: 15, maxSize:32, flakeCount:4, maxSpeed: 3, minSpeed: 2});
...
if(biicore.alert && Object.keys(biicore.alert).length > 0 && biicore.alert.status == 1) {
	setTimeout(function(){
		Swal.fire({
			...
			focusCancel: true,
			cancelButtonText: 'Táº¯t thĂ´ng bĂ¡o',
		});
...
if(biicore.bgMusic){
	...
	<div class="bii-player">
		<div onclick="playPause();" class="bii-player-secondary"><div class="bii-player-secondary-content">Ấn vào để phát nhạc</div></div>
...
if(!biicore.isPremium) {
	...
	<div class="bii-logo"><div class="bii-logo-secondary"><div class="bii-logo-secondary-content">Ná»n táº£ng táº¡o Website Ä‘Ă¡m cÆ°á»›i miá»…n phĂ­!</div></div><a href="${biicore.coreSite}/wedding" target="_blank"><img src="${biiLogo}" /></a></div>
	<div class="bii-footer" style="z-index:9999;background-color: #000;border-top: 1px solid #df4759;color: #fff;text-align: center;letter-spacing: 1px;bottom: 0;width: 100%;font-size: 13px;">
		<div class="container">
			<a style="color:#fff;padding: 7px 0 9px;display: block;" href="https://biihappy.com/wedding" target="_blank">
				<span><img width="16" src="${biiLogo}" style="border: 1px solid #fff;border-radius: 50%;width:16px!important;" /> IWedding</span> | <span class="show-desktop">Ná»n táº£ng táº¡o website Ä‘Ă¡m cÆ°á»›i miá»…n phĂ­ tá»« Biihappy</span><span class="show-mobile">Táº¡o website Ä‘Ă¡m cÆ°á»›i miá»…n phĂ­</span>
